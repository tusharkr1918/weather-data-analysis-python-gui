...
def get_daily_forecast(api_key, location, days):
    ...
    params = {"key": api_key, "q": location, "days": days, "aqi": "yes"}
    response = None
    try:
...
def analyze_and_plot(api_key, location, days, size):
    ...
    dates = [
        datetime.strptime(day["date"], "%Y-%m-%d").strftime("%d-%m-%Y")
        for day in forecast_data["forecast"]["forecastday"]
    ]
    max_temps = [
        day["day"]["maxtemp_c"] for day in forecast_data["forecast"]["forecastday"]
    ]
    min_temps = [
        day["day"]["mintemp_c"] for day in forecast_data["forecast"]["forecastday"]
    ]
    avg_temps = [
        day["day"]["avgtemp_c"] for day in forecast_data["forecast"]["forecastday"]
    ]
    precip = [
        day["day"]["totalprecip_mm"] for day in forecast_data["forecast"]["forecastday"]
    ]
