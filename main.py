...
class WeatherApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.progress_bar = ctk.CTkProgressBar(self.left_frame, mode="determinate", height=3, progress_color="grey11")
        self.progress_bar.set(1)
        self.progress_bar.grid(row=3, column=0, columnspan=2, sticky="we", pady=(5, 5), padx=15)

        self.bottom_box = ctk.CTkTextbox(self.left_frame, height=18, font=ctk.CTkFont(family='Constantia', size=14))
        self.bottom_box.insert("0.0", "")
        self.bottom_box.configure(state="disabled", text_color="orange")
        self.bottom_box.grid(row=4, column=0, columnspan=2, sticky="nsew", padx=15, pady=(5, 15))
        # Set row weights
        ...
        self.rounded_frame = ctk.CTkLabel(self.main_box, width=530, height=360, bg_color="transparent",
                                          fg_color="gray11", corner_radius=10, text="")
        ...
        self.bottom_frame = ctk.CTkFrame(master=self, fg_color='grey11', corner_radius=0)
        self.bottom_frame.grid(row=1, column=0, columnspan=2, sticky="ew")

        # Create a button inside the frame and use grid
        self.api_btn = ctk.CTkButton(self.bottom_frame, text="API KEY", fg_color="gray26",
                                     font=ctk.CTkFont(family='Constantia', size=10, weight="bold"), corner_radius=5,
                                     command=self.open_api_manager, height=10, width=100)
        self.api_btn.grid(row=0, column=0, sticky="news", padx=(710, 15), pady=2)
    ...
    def add_tabview_frame(self):
        ...
        self.add_password = ctk.CTkEntry(self.tabview_addbtn, placeholder_text="Password", font=ctk.CTkFont(family='Constantia', size=15))
        self.add_password.grid(sticky="nsew", pady=(0, 10))

        self.add_submit = ctk.CTkButton(self.tabview_addbtn, text="Submit",
    ...
    def configure_api_manager_widgets(self):
        ...
        self.api_label = ctk.CTkLabel(self.top, text="[Manage API keys]\nAdd a new key or load an existing one.",
                                      width=15,
                                      font=ctk.CTkFont(family='Constantia', size=15), wraplength=340)
        self.api_label.grid(row=0, column=0, pady=(20, 0), padx=40, sticky="new")

        self.manager_tabview = ctk.CTkTabview(self.top)
        self.manager_tabview.segmented_button.configure(font=ctk.CTkFont(family='Constantia', size=12))
        self.manager_tabview.grid(row=1, column=0, columnspan=2, padx=20, pady=(5, 20), sticky="nsew")
    ...
    def update_app(self):
        ...
        def analyze_and_plot_thread():
            ...
            if figure is not None and data is not None and buffer is not None:
                ...
                buffer.close()
                self.progress_bar.configure(mode="determinate", progress_color="chartreuse1")
                ...
                self.update_ui_with_error({"error": {"code": 503, "message": "No internet connection."}})
                self.progress_bar.configure(mode="determinate", progress_color="red")
    ...
    def update_ui_with_data(self, data, figure):
        plotted_image = ctk.CTkImage(dark_image=figure, light_image=figure, size=(500, 360))
        ...
        self.update_text_box(self.weather_box, f"{data['current']['temp_c']}°C, {data['current']['condition']['text']}")

    def update_ui_with_error(self, data):
        error_message = f"code: {data['error']['code']}\nmessage: {data['error']['message']}"
